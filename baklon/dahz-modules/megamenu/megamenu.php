...
if( !class_exists('Dahz_Framework_Megamenu') ){
	...
	Class Dahz_Framework_Megamenu {
		...
		function __construct(){
			...
			add_filter( 'dahz_framework_localize', array( $this, 'dahz_framework__megamenu_localize' ) );
		...
		*/
		public function dahz_framework_megamenu_builder( $items ){
			...
			$items['mega_menu'] = array(
				'title'				=> esc_html__( 'Primary Menu', 'baklon' ),
				'description'		=> esc_html__( 'Display megamenu', 'baklon' ),
		...
		public function dahz_framework_render_mobile_megamenu(){
			...
			$params_topbar = array(
				...
				'fallback_cb'	=> true,
				'items_wrap'	=> '<ul id="%1$s" class="de-mobile-nav de-mobile-primary__nav uk-nav-default uk-nav-parent-icon" data-uk-nav="multiple:false;toggle:> .mobile-toggle">%3$s</ul>',
				'echo'			=> false,
			);
		...
		*/
		...
		public function dahz_framework__megamenu_localize( $localize ){
			
			$localize['megamenu'] = array(
				'isOverride'	=> is_singular( 'page' ),
				'pageID'		=> is_singular( 'page' ) ? get_the_ID() : false,
			);
