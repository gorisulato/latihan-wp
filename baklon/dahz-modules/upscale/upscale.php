...
if ( !function_exists( 'dahz_framework_custom_password_protected' )) {
	 ...
	 */
	 function dahz_framework_custom_password_protected( $color = '' ) {
		...
		' . esc_html__( 'This content is password protected. To view it please enter your password below:', 'baklon' ) . '
		</p><p><label class="pass-label" for="' . $label . '">' . esc_html__( 'Password:', 'baklon' ) . ' <input name="post_password" id="' . $label . '" type="password" size="20" /></label>
		</p><input type="submit" name="Submit" class="uk-button uk-button-default button" value="' . esc_attr__( 'Submit', 'baklon' ) . '" /></form>
		';
		return $form_render;
	}
...
if ( !function_exists( 'dahz_baner_header_search_modal' ) ) {
	...
	function dahz_baner_header_search_modal( $output ) {
		...
		$output = sprintf(
			...
			',
			esc_attr__( 'Start typing here...', 'baklon' ), # 1
			esc_url( home_url() ), # 2
...
if ( !function_exists( 'dahz_pabu_header_search_list' ) ) {
	...
	function dahz_pabu_header_search_list( $list_output, $thumbnail, $post_link, $title, $categories ) {
		...
		$list_output = sprintf(
			...
			$title, # 3
			$categories, # 4
			get_post_type() == 'post' ? __( 'By', 'baklon' ) : '', # 5
			get_post_type() == 'post' ? dahz_framework_get_post_meta_author() : '', # 6
			!empty( dahz_framework_get_post_meta_categories() ) ? __( 'In', 'baklon' ) : '', # 7
...
if ( !function_exists( 'dahz_pabu_search_notfound' ) ) {
	...
	function dahz_pabu_search_notfound( $options ) {
		...
		$options = array_merge(
			...
			array(
				'language' => array(
					'emptyMessage' => __( '<h4>OH NO! No result were found</h4>', 'baklon' )
				)
			)
		);
