	...
	Class Dahz_Framework_Blog_Single{
		...
		public function dahz_framework_blog_single_init( $path ){
			...
			dahz_framework_register_customizer(
				...
				array(
					'id'	=> 'blog_single',
					'title' => array( 'title' => esc_html__( 'Single Post', 'baklon' ), 'priority' => 3 ),
					'panel'	=> 'blog'
				),
				array()
			);
			...
			echo "<div class='de-single__content-post-wrapper uk-width-1-1@m " . esc_attr( $dataFirstLetter )."'>";
				the_content(
					...
					/* translators: %s: Name of current post. */
						wp_kses(
							__( 'Read More %s <span class="meta-nav">&rarr;</span>', 'baklon' ),
		...
		public function dahz_framework_blog_single_lazy_related(){
			...
			if ( $related ){
				...
				$related_html = sprintf(
					...
					',
					sprintf( _n( 'Related Article', 'Related Articles', $count, 'baklon' ), $count ),
					$related_loop_html,
					$image_cover,
		...
		public function dahz_framework_blog_single_comment_button() {
			return '<button type="submit" id="submit" class="uk-button uk-button-default">'. esc_html__( 'Submit', 'baklon' ) .'</button>';
		}
