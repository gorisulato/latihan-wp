	...
	Class Dahz_Framework_Header_Button {
		...
		public function dahz_framework_header_button_init( $path ){
			...
			dahz_framework_register_customizer(
				...
				array(
					'id'	=> 'header_button',
					'title'	=> array( 'title' => esc_html__( 'Button Header', 'pabu' ), 'priority' => 19 ),
					'panel'	=> 'header'
				),
				array()
			);
		...
		public function dahz_framework_header_item_button( $items ) {
			...
			$items['button'] = array(
				'title'				=> esc_html__( 'Button', 'pabu' ),
				'description'		=> esc_html__( 'Display Button in Header', 'pabu' ),
		...
		public function dahz_framework_header_button_element( $builder_type, $section, $row, $column ){
			...
			$button_link = dahz_framework_get_option( 'header_button_link' );
			
			if( empty( $button_text ) ){return;}
			
			echo sprintf(
				'
				<a href="%3$s" class="de-header-button uk-button uk-button-default de-btn--fill" target="%2$s">%1$s</a>
				',
				esc_html( $button_text ),
				esc_attr( dahz_framework_get_option( 'header_button_target', '_blank' ) ),
				strpos( $button_link, '#' ) == false ? esc_url( $button_link ) : $button_link
			);
