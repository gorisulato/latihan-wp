	...
	Class Dahz_Framework_Page_Title {
		...
		public function dahz_framework_remove_label_title( $title ) {
			...
			if ( is_category() ) {
				...
				$title = get_the_date( _x( 'Y', 'yearly archives date format', 'baklon' ) );

			} elseif ( is_month() ) {

				$title = get_the_date( _x( 'F Y', 'monthly archives date format', 'baklon' ) );

			} elseif ( is_day() ) {

				$title = get_the_date( _x( 'F j, Y', 'daily archives date format', 'baklon' ) );
		...
		public function dahz_framework_set_page_title( $styles = '' ) {
			...
			$this->registered_page_title = apply_filters( 'dahz_framework_register_page_title', $this->registered_page_title, $this );
		...
		public function dahz_framework_shop_page_title( $page_title ) {
			...
			if ( class_exists( 'Woocommerce' ) && is_shop() ) {
				...
				$page_title = array(
					...
					'breadcrumb'					=> $woocommerce_breadcrumb,
					'title'							=> !empty( $shop ) ? get_the_title( $shop ) : __( 'Shop', 'baklon' ),
				...
				if ( $layout !== 'disable' ) {
					dahz_framework_get_template(
						...
						array(
							...
							'layout'			=> $layout,
							'title'				=> isset( $page_titles['title'] ) ? $page_titles['title'] : '',
							'title_class'		=> isset( $page_titles['title_class'] ) ? $page_titles['title_class'] : '',
							'description'		=> isset( $page_titles['description'] ) ? $page_titles['description'] : '',
