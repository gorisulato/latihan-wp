	...
	class Dahz_Framework_Sticky_Header {
		...
		public function dahz_framework_header_sticky_init( $path ) {
			...
			dahz_framework_register_customizer(
				...
				array(
					'id'	=> 'sticky_header',
					'title'	=> array( 'title' => esc_html__( 'Sticky Header', 'baklon' ), 'priority' => 6 ),
					'panel'	=> 'header'
				),
				array()
			);
			...
			if ( !empty( $sticky_sections ) && !empty( $different ) ) {
				...
				$output = sprintf(
					...
					<div id="de-header-horizontal-desktop" class="uk-visible@m ds-header--wrapper de-header__wrapper de-header default uk-position-z-index">
						<div class="de-header__sticky--wrapper" data-header-sticky-box-shadow="%2$s" data-header-sticky-offset="%3$s" data-header-sticky-top="%4$s">
							%1$s
						</div>
					</div>
					',
					...
					apply_filters( 'dahz_framework_header_sticky_top', false )
				);
