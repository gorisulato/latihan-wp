<div class="ds-product-single__navigation-control">
	<?php if ( dahz_framework_get_option( 'single_woo_is_prev_next_product', true ) ) : ?>
		<div class="ds-product-single__navigation-control__post">
			...
			?>
			<?php if ( $prev_post ) : ?>
			<a aria-label="<?php echo esc_attr_e( 'Previous Product', 'baklon' ); ?>" href="<?php echo esc_attr( get_permalink( $prev_post->ID ) ); ?>" class="uk-icon-link left" data-uk-icon="icon: chevron-left;ratio: 1">
			...
			<?php endif; ?>
			<?php if ( $next_post ) : ?>
			<a aria-label="<?php echo esc_attr_e( 'Next Product', 'baklon' ); ?>" href="<?php echo esc_attr( get_permalink( $next_post->ID ) ); ?>" class="uk-icon-link right" data-uk-icon="icon: chevron-right;ratio: 1">
