...
add_filter( 'wc_add_to_cart_message_html', 'your_add_to_cart_message', 10, 2 );

function your_add_to_cart_message( $message, $products ) {
	...
	foreach ( $products as $product_id => $qty ) {
		...
		$titles[] = ( $qty > 1 ? absint( $qty ) . ' &times; ' : '' ) . sprintf( _x( '&ldquo;%s&rdquo;', 'Item name in quotes', 'baklon' ), strip_tags( get_the_title( $product_id ) ) );
		$count   += $qty;
	}
    ...
    /* translators: %s: product name */
	$added_text = sprintf( _n( '%s has been added to your cart.', '%s have been added to your cart.', $count, 'baklon' ), wc_format_list_of_items( $titles ) );

    if ( get_option( 'woocommerce_cart_redirect_after_add' ) == 'yes' ) :
        $message = sprintf( '%s<a href="%s" class="uk-button uk-button-text">%s</a>', __( 'Successfully added to cart.', 'baklon' ), esc_url( get_permalink( woocommerce_get_page_id( 'shop' ) ) ), __( 'Continue Shopping', 'baklon' ) );
    else :
        $message = sprintf( '<a href="%s" class="wc-forward uk-button uk-button-text">%s</a> %s', esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'View cart', 'baklon' ), esc_html( $added_text ) );
    endif;
    return $message;
}
	...
	function dahz_framework_render_badge_sale( $class = 'entry-badge' ) {
		...
		if ( $product->is_on_sale() ) {
			...
			if ( 'text' == $badge_type ) {
				...
				echo apply_filters( 'woocommerce_sale_flash', sprintf( '<p class="%s sale">%s</p>', esc_attr( $class ), esc_html__( 'Sale', 'baklon' ) ), $post, $product );
...
if ( !function_exists( 'dahz_framework_render_badge_outofstock' ) ) {
	...
	function dahz_framework_render_badge_outofstock( $class = 'entry-badge' ) {
		...
		if ( !$product->is_in_stock() ) {
			...
			echo sprintf( '<p class="%s out-of-stock">%s</p>', esc_attr( $class ), esc_html__( 'Out of Stock', 'baklon' ) );
...
if ( !function_exists( 'dahz_framework_render_product_view_all' ) ) {
	...
	function dahz_framework_render_product_view_all() {
		...
		printf( '<p><a class="uk-link" href="%s">%s</a></p>', esc_url( add_query_arg( 'view', 'all' ) ), esc_html( 'View All', 'baklon' ) );
...
if ( !function_exists( 'dahz_framework_render_product_filter_button' ) ) {
	...
	function dahz_framework_render_product_filter_button() {
		...
		printf( '<p class="de-shop__filter-button ds-filter-button--open"><a href="#" data-uk-toggle="target: .sidebar--header;cls: sidebar--visible">%s</a></p>', esc_html( 'Filter', 'baklon' ) );
