...
if ( !class_exists( 'Dahz_Framework_Single_Page' ) ) {
	...
	class Dahz_Framework_Single_Page {
		...
		function __construct() {
			...
			add_filter( 'dahz_framework_header_sticky_top', array( $this, 'dahz_framework_header_sticky_top' ) );
		...
		public function dahz_framework_is_ajax_menu_mobile(){
			
			return ( !empty( $_POST['is_override'] ) && ( defined('DOING_AJAX') && DOING_AJAX ) && !empty( $_POST['page_id'] ) );
		...
		public function dahz_framework_overrided_menu( $override_menu ) {
			...
			if ( is_singular( 'page' ) || $this->dahz_framework_is_ajax_menu_mobile() ) {
				
				$page_id = $this->dahz_framework_is_ajax_menu_mobile() ? $_POST['page_id'] : get_the_ID();
		...
		public function dahz_framework_header_sticky_top( $is_sticky_offset_top ){
			
			$header_remove_sticky_shadow = dahz_framework_get_meta( get_the_ID(), 'dahz_meta_page', 'disable_sticky_top_offset', 'off' );
			
			$is_sticky_offset_top = $header_remove_sticky_shadow !== 'off' ? true : false;
			
			return $is_sticky_offset_top;
