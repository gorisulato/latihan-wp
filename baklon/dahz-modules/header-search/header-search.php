...
class Dahz_Framework_Header_Search {
	...
	public function dahz_framework_header_search_init( $path ) {
		...
		dahz_framework_register_customizer(
			...
			array(
				'id'	=> 'header_search',
				'title'	=> array( 'title' => esc_html__( 'Search', 'baklon' ), 'priority' => 14 ),
				'panel'	=> 'header'
			),
			array()
		);
	...
	public function dahz_framework_header_item_search( $items ) {
		$items['search'] = array(
			'title'				=> esc_html__( 'Search', 'baklon' ),
			'description'		=> esc_html__( 'Display ajax search', 'baklon' ),
	...
	public function dahz_framework_header_elem_search( $builder_type, $section, $row, $column ) {
		...
		switch( $search_style ) {
			case 'text':
				$search_link_content = '<span>' . __( 'Search', 'baklon' ) . '</span>';
				break;
			case 'icon':
			case 'icon-right':
				...
				break;
			case 'icon-text':
				$search_link_content = '<span class="uk-margin-small-right">' . __( 'Search', 'baklon' ) . '</span>' . $search_icon;
				break;
			default:
				$search_link_content = $search_icon . '<span class="uk-margin-small-left">' . __( 'Search', 'baklon' ) . '</span>';
				break;
		}
	...
	public function dahz_framework_render_search_modal() {
		...
		if ( isset( $dahz_framework->builder_items['search'] ) || isset( $dahz_framework->builder_items['search_mobile'] ) ) {
			...
			$search_output = sprintf(
				...
				',
				class_exists( 'WooCommerce' ) ? esc_attr__( 'Search by brand, name, etc', 'baklon' ) : esc_attr__( 'Search', 'baklon' )
			);
