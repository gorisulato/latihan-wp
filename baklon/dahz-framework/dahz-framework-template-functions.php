...
if ( !function_exists( 'dahz_framework_header_lists' ) ) {
	function dahz_framework_header_lists( $items ) {
		$items['logo'] = array(
			'title'				=> esc_html__( 'Logo', 'baklon' ),
			'description'		=> esc_html__( 'Display site logo', 'baklon' ),
...
if ( !function_exists( 'dahz_framework_footer_lists' ) ) {
	function dahz_framework_footer_lists( $items ) {
		$items['site_copyright'] = array(
			'title'				=> esc_html__( 'Site Copyright', 'baklon' ),
			'description'		=> esc_html__( 'Display copyright info', 'baklon' ),
		...
		$items['footer_description'] = array(
			'title'				=> esc_html__( 'Footer Description', 'baklon' ),
			'description'		=> esc_html__( 'Display description text', 'baklon' ),
...
if ( !function_exists( 'dahz_framework_headermobile_lists' ) ) {
	function dahz_framework_headermobile_lists( $items ) {
		$items['logo_mobile'] = array(
			'title'				=> esc_html__( 'Logo', 'baklon' ),
			'description'		=> esc_html__( 'Display mobile site logo', 'baklon' ),
			'render_callback'	=> 'dahz_framework_header_logo',
			'section_callback'	=> 'logo_and_site_identity',
			'is_repeatable'		=> false
		);
...
if ( !function_exists( 'dahz_framework_skip_link' ) ) {
	function dahz_framework_skip_link() {
		echo '<a class="skip-link screen-reader-text de-skip-link" href="#de-archive-content">' . esc_html__( 'Skip to content', 'baklon' ).'</a>';
	}
}
				...
				'site_logo_image'	=> apply_filters( 'dahz_framework_logo_normal',
					...
					:
						sprintf(
							...
							sprintf( esc_html__( '%s', 'baklon' ), get_bloginfo( 'description' ) )
						)
				)
			)
...
if ( !function_exists( 'dahz_framework_footer_site_copyright' ) ) {
	function dahz_framework_footer_site_copyright() {
		dahz_framework_get_template(
			...
			array(
				'site_info' => dahz_framework_get_option( 'footer_element_footer_site_info', __( '&#169;2018 Baklon - Election &amp; Political WordPress Theme', 'baklon' ) ),
	...
	function dahz_framework_pagination( $pagination_type = "number", $masonry = false, $class="", $is_echo = true ) {
		...
		switch ( $pagination_type ) {
			case 'prev_next':
				...
				$pagination_output .= get_previous_posts_link( '<h6><span data-uk-icon="chevron-left"></span>' . __( 'Previous', 'baklon' ) . '</h6>' );

				$pagination_output .= '</li><li>';

				$pagination_output .= get_next_posts_link( '<h6>' . __( 'Next', 'baklon' ) . '<span data-uk-icon="chevron-right"></span></h6>' );
			...
			case 'infinity-scroll':
				...
				$pagination_output = sprintf(
					...
					esc_attr( $max ),
					esc_attr( $masonry ),
					esc_html__( 'To Infinity and Beyond', 'baklon' ),
					!empty( $class ) ? "{$class} " : ''
				);
			break;
			case 'load-more':
...
if ( !function_exists( 'dahz_framework_post_navigation' ) ) {
	function dahz_framework_post_navigation() {
		...
		if ( ( !empty( $prev ) && $prev_post != '' ) && ( $prev != $current_url . '/' || $next != $current_url . '/' ) ) {
			...
			$title    = esc_html__( 'Previous Reading', 'baklon' );
		...
		if ( ( !empty( $next ) && $next_post != '' ) && ( $prev != $current_url . '/' || $next != $current_url . '/' ) ) {
			...
			$title    = esc_html__( 'Next Reading', 'baklon' );
			$dv_html .= apply_filters(
			...
			<article class="comment-wrapper uk-comment">
					<header class="uk-comment-header uk-grid-medium uk-flex-middle uk-grid" data-uk-grid>
						<div class="uk-width-auto">
							<?php echo get_avatar( $comment, 80, '', '', array( 'class' => 'uk-comment-avatar uk-margin-medium-right' ) ); ?>
						</div>
								...
								<ul class="uk-comment-meta uk-subnav uk-subnav-divider uk-margin-remove-top">
									...
									<?php 
										$reply_link = get_comment_reply_link(
												...
												'reply_text' => esc_html__( 'Reply', 'baklon' ),
												'add_below'  => 'comment',
												'depth'      => $depth,
										...
										if( !empty( $reply_link ) ) : 
											echo sprintf( '<li>%s</li>', $reply_link );
										endif;
									?>
									<?php if ( is_user_logged_in() ) { ?>
										<li>
											<?php edit_comment_link( __( 'Edit', 'baklon' ), '<span class="de-comments__edit-btn">', '</span>' ); ?>
										</li>
									<?php }	?>
								</ul>
						</div>
					...
					<div class="comment-content uk-comment-body">
						...
						<?php if ( $comment->comment_approved == '0' ) : ?>
							<p><?php esc_html_e( 'Your comment is awaiting moderation.', 'baklon' ); ?></p>
						<?php endif; ?>
					</div>
			</article>
		</li>
...
if ( !function_exists( 'dahz_framework_render_post_content' ) ) {
	function dahz_framework_render_post_content( $id ) {
		the_content( sprintf(
			esc_html__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'baklon' ),
			get_the_title()
		) );
...
if ( !function_exists( 'dahz_framework_render_404_widget' ) ) {
	function dahz_framework_render_404_widget() {
		...
		$archive_content = '<p>' . sprintf( esc_html__( 'Try looking in the monthly archives. %1$s', 'baklon' ), convert_smilies( ':)' ) ) . '</p>';
...
if( ! function_exists( 'dahz_framework_get_sticky_post' ) ){
	...
	function dahz_framework_get_sticky_post( $id = null ) {
		...
		return is_sticky( $id ) ? '<span data-uk-icon="star"></span>' : '';
	...
	function dahz_framework_get_post_meta_comment( $id = null ) {
		...
		return sprintf(
			...
			$comments_count > 0 
				?
				sprintf(
					esc_html( _nx( '%1$s Comment', '%2$s Comments', $comments_count, 'approved comments count ', 'baklon' ) ),
					'1',
					number_format_i18n( $comments_count )
				)
				:
				esc_html__( 'Leave a reply', 'baklon' )
		);
...
if( ! function_exists( 'dahz_framework_excerpt_is_read_more' ) ){
	...
	function dahz_framework_excerpt_is_read_more( $content = '', $num_words = 40 ) {
		...
		if ( strpos( _x( 'words', 'Word count type. Do not translate!', 'baklon' ), 'characters' ) === 0 && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
...
if( ! function_exists( 'dahz_framework_get_excerpt' ) ){
	...
	function dahz_framework_get_excerpt( $args = array() ) {
		...
		if ( $args['num_words'] >= 0 ) {
			...
			$the_content = ! has_excerpt( get_the_ID() ) ? !empty( $args['content'] ) ? $args['content'] : get_the_content() : get_the_excerpt();
			...
			$the_content = !empty( $args['content'] ) ? $args['content'] : get_the_content();
		...
		return dahz_framework_get_template_html(
			...
			array(
				'content'		=> $content,
				'is_read_more'	=> $args['show_readmore'] && $args['num_words'] >= 0 && dahz_framework_excerpt_is_read_more( $the_content, $args['num_words'] ) ? true : false,
