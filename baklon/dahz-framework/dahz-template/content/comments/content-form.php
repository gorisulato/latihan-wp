...
$args = array(
		'comment_field'	=> '
			<p class="comment-form-comment">
				<label for="comment">' . esc_html__( 'Your Comment', 'baklon' ) . '</label>
		...
		'label_submit'	=> esc_html__('Post Comment', 'baklon' ),
		'comment_field'	=> '<div class="uk-width-1-1"><label for="comment">' . esc_html__( 'Comment', 'baklon' ) . '</label><textarea id="comment" class="uk-width-1-1" name="comment" cols="45" rows="8" aria-required="true"></textarea></div>',
		'comment_notes_before' => '<div class="uk-width-1-1"><p class="comment-notes uk-width-1-1">' . __( 'Your email address will not be published.', 'baklon' ) . ( $req ? __( 'Required fields are marked *', 'baklon' ) : '' ) . '</p></div>',
		'fields'		=> apply_filters( 'comment_form_default_fields', array(
			'author'=> '<p class="uk-width-1-2 comment-form-author">
							<label for="author">' . __( 'Name', 'baklon' ) .
			...
			'url'	=> '<p class="uk-width-1-1 comment-form-url">
						<label for="url">' . __( 'Website', 'baklon' ) . '</label>
						<input id="url"
							 name="url"
							 type="url"
