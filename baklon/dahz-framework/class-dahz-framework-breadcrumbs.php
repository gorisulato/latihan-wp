	...
	Class Dahz_Framework_Breadcrumbs {
		...
		public function dahz_framework_breadcrumbs_init(){
			...
			$this->output .= $this->dahz_framework_wrap_breadcrumbs(
				array(
					'url'	=> get_home_url(),
					'title'	=> esc_attr__( 'Home', 'baklon' ),
					'text'	=> esc_html__( 'Home', 'baklon' ),
				)
			);
			...
			if ( $this->dahz_framework_is_archive_only() ) {
				...
				$this->output .= $this->dahz_framework_wrap_breadcrumbs(
					array(
						'text'	=> sprintf('%3$s %1$s %2$s', esc_html( get_the_time('F') ), esc_html( get_the_time('Y') ), esc_html__( 'Month:', 'baklon' ) ),
					),
					true
				);
				...
				$this->output .= $this->dahz_framework_wrap_breadcrumbs(
					array(
						'text'	=> sprintf('%2$s %1$s', esc_html( get_the_time('Y') ), esc_html__( 'Year:', 'baklon' ) ),
					),
					true
				);
			} else if ( is_author() ) {
				...
				$this->output .= $this->dahz_framework_wrap_breadcrumbs(
					array(
						'text'	=> sprintf('%2$s %1$s',esc_html( $userdata->display_name ), esc_html__( 'Author / ', 'baklon' ) ),
					),
					true
				);
				// Display author name
				...
				$this->output .= $this->dahz_framework_wrap_breadcrumbs(
					array(
						'text'	=> sprintf('%2$s %1$s', esc_html( get_query_var('paged' ) ), esc_html__('Page', 'baklon' ) ),
					),
					true
				);
			} else if ( is_search() ) {
				...
				$this->output .= $this->dahz_framework_wrap_breadcrumbs(
					array(
						'text'	=> sprintf('%2$s %1$s', esc_html( get_search_query() ), esc_html__('Search results for: ', 'baklon' ) ),
					),
					true
				);
				...
				$this->output .= $this->dahz_framework_wrap_breadcrumbs(
					array(
						'text'	=> esc_html__('Error 404', 'baklon' ),
					),
					true
				);
		...
		public function dahz_framework_wrap_breadcrumbs( $args = array(), $is_active = false ){
			...
			return apply_filters( 
				'dahz_framework_breadcrumb_items', 
				!$is_active 
					?
				sprintf(
					'<li>
