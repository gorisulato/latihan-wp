...
if ( !class_exists( 'Dahz_Framework_Init' ) ) {
	...
	Class Dahz_Framework_Init {
		...
		public function dahz_framework_get_frontend_scripts() {
			...
			return apply_filters(
				...
				array(
					...
					'dahz'				=> array(
						'settings'		=> array( 'dahz-framework-script', DAHZ_FRAMEWORK_THEME_URI . '/assets/dist/js/dahz-framework.min.js', array( 'jquery', 'underscore' ), null, true ),
						'enqueue'		=> false
					),
					'sticky-sidebar'	=> array(
						'settings'		=> array( 'theia-script', DAHZ_FRAMEWORK_THEME_URI . '/assets/dist/js/plugins/theia-sticky-sidebar.min.js', array( 'jquery', 'resize-sensor' ), null, true ),
						'enqueue'		=> false
					),
					'resize-sensor'		=> array(
		...
		public function dahz_framework_setup() {
			 ...
			 */
			load_theme_textdomain( 'baklon', get_template_directory() . '/languages' );
			...
			register_nav_menus(
				array(
					'secondary_menu' => esc_html__( 'Secondary Menu', 'baklon' ),
				)
			);
		...
		public function dahz_framework_widgets_init() {
			...
			$default_sidebar = apply_filters( 'dahz_framework_widgets', array ( 'default' =>
				...
				array(
					...
					'name'			=> esc_html__( 'Sidebar', 'baklon' ),
					'id'			=> 'sidebar-1',
					'description'	=> esc_html__( 'Add widgets here.', 'baklon' ),
					'before_widget'	=> '<div id="%1$s" class="widget %2$s uk-margin-medium">',
					'after_widget'	=> '</div>',
					'before_title'	=> '<h6 class="uk-heading-bullet widgettitle"><span>',
					'after_title'	=> '</span></h6>',
		...
		public function dahz_framework_enqueue_admin_scripts() {
			...
			wp_enqueue_script(	'wp-color-picker-alpha'	 , DAHZ_FRAMEWORK_THEME_URI . '/assets/dist/js/plugins/wp-color-picker-alpha.min.js', array( 'wp-color-picker' ), null, true );
			...
			wp_localize_script(
				...
				apply_filters(
					...
					array(
						...
						'language'				=> array(
							'emptyMessage'		=> esc_html__( 'We can&rsquo;t find anything', 'baklon' )
						)
					)
