	...
	Class Dahz_Framework_Customizer_Init{
		...
		public function dahz_framework_customize_controls_enqueue_scripts(){
			...
			wp_localize_script(
				...
				array(
					'messages' => array(
						'emptyImport'			=> esc_html__( 'Please choose a file to import.', 'baklon' ),
			...
			wp_localize_script(
				...
				array(
					'notices'		=> array(
						'deletedMergedScripts'	=> esc_html__( 'All Scripts Has Been Deleted', 'baklon' )
					),
					'ajaxURL' 		=> admin_url( 'admin-ajax.php' ),
					'isDevelopMode'	=> DAHZ_FRAMEWORK_DEVELOP_MODE,
					'messages'		=> array(
						'errorSavePresetIncomplete' => esc_html__( 'Please Complete The Form', 'baklon' ),
						'itemNotExist'				=> esc_html__( 'This Item Not Exist', 'baklon' ),
					),
					'items'			=> array(
		...
		public function dahz_framework_customize_builder_save_preset(){
			...
			if ( empty( $preset['name'] ) ) {
				...
				echo json_encode( array( 'status' => 0, 'message' => esc_html__( "Name is required", 'baklon' ) ) );
			...
			if ( empty( $preset['value'] ) || !is_array( $preset['value'] ) ) {
				...
				echo json_encode( array( 'status' => 0, 'message' => esc_html__( "Value is not valid", 'baklon' ) ) );
			...
			if ( isset( $customize_builder_presets[$preset['name']] ) && $preset['action_type'] !== 'replace' ) {
				...
				echo json_encode( array( 'status' => 2, 'message' => esc_html__( "Preset is already exist", 'baklon' ) ) );
			...
			echo json_encode(
				array(
					'status' 	=> 1,
					'message' 	=> esc_html__( "Preset successfully saved", 'baklon' ),
		...
		public function dahz_framework_customize_builder_remove_preset(){
			...
			if ( isset( $customize_builder_presets[$preset['name']] ) && $preset['action_type'] === 'delete' ) {
				...
				echo json_encode( array( 'status' => 1, 'message' => esc_html__( "Preset successfully deleted", 'baklon' ) ) );
