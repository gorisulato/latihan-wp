...
class Kirki_Control_Image extends Kirki_Control_Base {
	...
	protected function content_template() {
		...
		data.choices.labels = _.defaults( data.choices.labels, {
			select: '<?php esc_attr_e( 'Select image', 'baklon' ); ?>',
			change: '<?php esc_attr_e( 'Change image', 'baklon' ); ?>',
			'default': '<?php esc_attr_e( 'Default', 'baklon' ); ?>',
			remove: '<?php esc_attr_e( 'Remove', 'baklon' ); ?>',
			placeholder: '<?php esc_attr_e( 'No image selected', 'baklon' ); ?>',
			frame_title: '<?php esc_attr_e( 'Select image', 'baklon' ); ?>',
			frame_button: '<?php esc_attr_e( 'Choose image', 'baklon' ); ?>',
		} );
		#>
		<label>
		...
		<div class="image-wrapper attachment-media-view image-upload">
			<# if ( data.value['url'] || '' !== url ) { #>
				<div class="thumbnail thumbnail-image">
					<img src="{{ url }}" alt="<?php esc_attr_e( 'Background Image', 'baklon' );?>" />
				</div>
			<# } else { #>
