...
<form method="post">
	<?php if ( $_this->is_possible_upgrade() ) { ?>
		<p><?php esc_html_e( 'It looks like you already have content installed on _this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'baklon' ); ?></p>
	<?php } else { ?>
		<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, _this content can be managed from the WordPress admin dashboard. ', 'baklon' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
	<?php } ?>
		...
		<?php foreach ( $_this->_content_default_get() as $slug => $default ) { ?>
			...
			<?php if( in_array( $slug, $post_types ) && $slug !== 'attachment' && $slug !== 'nav_menu_item' ) : ?>
				<tr class="envato_default_sub_content" data-sub-content="<?php echo esc_attr( $slug ); ?>">
					<td class="column-4 toggle-header"><?php esc_html_e( 'Click here to select manually', 'baklon' ); ?></td>
